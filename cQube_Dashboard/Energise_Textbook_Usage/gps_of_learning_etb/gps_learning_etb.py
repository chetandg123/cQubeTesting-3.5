...
class etb_content_plays_map_report():
    ...
    def check_navigation_from_dashboard(self):
        ...
        if 'etb-total-content-plays' in self.driver.current_url:
            print("GPS of Learning- ETB Report is displayed")
        else:
            print('GPS of Learning- ETB Report is not displayed')
    ...
    def check_report_home_page(self):
        ...
        if 'No data found' in self.driver.page_source:
            print("GPS of Learning- ETB Report is showing no data found!!!")
            count = count + 1
    ...
    def check_total_content_plays_legendcard(self):
        ...
        for i in range(5):
            ...
            if 'No data found' in self.driver.page_source:
                ...
                time.sleep(3)
                self.filename = self.p.get_download_dir() + self.fname.etb_content_plays_file
    ...
    def check_total_time_spent_legendcard(self):
        ...
        for i in range(5):
            ...
            if 'No data found' in self.driver.page_source:
                ...
                self.driver.find_element(By.ID, Data.Download).click()
                time.sleep(3)
                self.filename = self.p.get_download_dir() + self.fname.etb_time_spent_file
                if os.path.isfile(self.filename) != False:
                    print(self.fname.etb_time_spent_file, 'is not downloaded ')
                    count = count + 1
                else:
                    df = pd.read_csv(self.filename)
                    size = len(df)
    ...
    def check_averaage_timespent_legendcard(self):
        ...
        for i in range(5):
            ...
            if 'No data found' in self.driver.page_source:
                ...
                if os.path.isfile(self.filename) != False:
                    print(self.fname.etb_average_time_spent_file, 'is not downloaded ')
                    count = count + 1
                else:
                    df = pd.read_csv(self.filename)
                    size = len(df)
