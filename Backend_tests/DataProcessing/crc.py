...
class CrcInspectionMaster(unittest.TestCase):

    def setUp(self):
        self.processor_name="crc_transformer"
        self.folder_name="inspection_master"
...
class CrcUserLocationMaster(unittest.TestCase):

    def setUp(self):
        self.processor_name="crc_transformer"
        self.folder_name="user_location_master"
        self.cQube_data_storage = "cQube_data_storage"
        self.cal = GetData()
        self.storage_type = self.cal.get_storage_type()
        self.cal.start_nifi_processor(self.cQube_data_storage)
    ...
    def test_user_location_master(self):
        if self.storage_type == "s3":
            result = self.cal.copy_file_to_s3(self.filepath, self.folder_name)
            time.sleep(10)
            if result.returncode == 0:
