...
from cQube_Dashboard.Teacher_Professional_Development.tpd_user_progress import enrollment_smoke_test
    ...
    def test_issue_20(self):
            ...
            if status == str(True):
                ...
                if 'No data found' in self.driver.page_source:
                    ...
                    smoke_test.addTests([
                        unittest.defaultTestLoader.loadTestsFromTestCase(
                            etb_gps_learning_smoke.gps_learning_tpd_smoke_suite
                        )])
                    p = pwd()
                    outfile = open(p.get_smoke_chart_tables_report(), "a")
                    runner1 = HTMLTestRunner.HTMLTestRunner(
